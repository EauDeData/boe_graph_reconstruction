...
def load_models(args, collator):
    ...
    graph_model = GraphConv(args.text_emb_size, 768, args.graph_in_channels,
                            args.graph_hidden_channels, args.graph_depth, args.graph_out_channels, num_categories=64,
                            device=args.device).to(args.device)

    return text_model, visual_model, graph_model

def main(args):

    (train_loader, test_loader), collator = load_datasets(args)
    text_model, visual_model, graph_model = load_models(args, collator)
    joint_model = JointModel(visual_model, text_model, graph_model)
    optimizer = Adam([
    ...
    for epoch in range(args.epoches):
        ...
        losses = eval_step(joint_model, train_loader, optimizer, lossf, logger, epoch)
        if logger:
